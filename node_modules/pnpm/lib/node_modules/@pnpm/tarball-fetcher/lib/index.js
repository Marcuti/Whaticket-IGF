...
async function fetchFromRemoteTarball(ctx, unpackTo, dist, opts) {
    try {
        ...
        switch (err.code) {
            case 'Z_BUF_ERROR':
                if (ctx.offline) {
                    throw new errorTypes_1.PnpmError('ERR_PNPM_CORRUPTED_TARBALL', `The cached tarball at "${opts.cachedTarballLocation}" is corrupted. Cannot redownload it as offline mode was requested.`);
                }
                logger_1.storeLogger.warn(`Redownloading corrupted cached tarball: ${opts.cachedTarballLocation}`);
                break;
            case 'EINTEGRITY':
                if (ctx.offline) {
                    throw new errorTypes_1.PnpmError('ERR_PNPM_BAD_TARBALL_CHECKSUM', `The cached tarball at "${opts.cachedTarballLocation}" did not pass the integrity check. Cannot redownload it as offline mode was requested.`);
                }
                logger_1.storeLogger.warn(`The cached tarball at "${opts.cachedTarballLocation}" did not pass the integrity check. Redownloading.`);
                break;
            case 'ENOENT':
                if (ctx.offline) {
