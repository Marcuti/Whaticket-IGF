...
async function recursive(allPkgs, input, opts, cmdFullName, cmd) {
    ...
    if (cmdFullName !== 'rebuild') {
        ...
        if (opts.lockfileDirectory && ['install', 'uninstall', 'update'].includes(cmdFullName)) {
            ...
            const mutatedImporters = await Promise.all(importers.map(async ({ buildIndex, prefix }, index) => {
        ...
        await Promise.all(pkgPaths.map((prefix) => limitInstallation(async () => {
            ...
            try {
                ...
                const localConfigs = await memReadLocalConfigs(prefix);
                const newManifest = await action(manifest, Object.assign({}, installOpts, localConfigs, { bin: path.join(prefix, 'node_modules', '.bin'), hooks, ignoreScripts: true, prefix, rawNpmConfig: Object.assign({}, installOpts.rawNpmConfig, localConfigs), storeController }));
