...
async function ensurePr(prConfig) {
    ...
    async function getBranchStatus() {
        if (!branchStatus) {
            branchStatus = await platform.getBranchStatus(branchName, config.requiredStatusChecks);
    ...
    if (config.automerge === true &&
        ...
        !config.forcePr) {
        logger.debug(`Branch is configured for branch automerge, branch status) is: ${await getBranchStatus()}`);
        if ((await getBranchStatus()) === 'pending' ||
    ...
    if (config.prCreation === 'status-success') {
        ...
        if ((await getBranchStatus()) !== 'success') {
            logger.debug(`Branch status is "${await getBranchStatus()}" - not creating PR`);
            return null;
        }
        ...
        if ((await getBranchStatus()) === 'pending' ||
            (await getBranchStatus()) === 'running') {
            logger.debug(`Branch status is "${await getBranchStatus()}" - checking timeout`);
            const lastCommitTime = await platform.getBranchLastCommitTime(branchName);
            const currentTime = new Date();
            const millisecondsPerHour = 1000 * 60 * 60;
            const elapsedHours = Math.round((currentTime.getTime() - lastCommitTime.getTime()) / millisecondsPerHour);
            if (elapsedHours < config.prNotPendingHours) {
                logger.debug(`Branch is ${elapsedHours} hours old - skipping PR creation`);
                return null;
            }
            logger.debug(`prNotPendingHours=${config.prNotPendingHours} threshold hit - creating PR`);
        }
        logger.debug('Branch status success');
    }
    ...
    try {
        if (existingPr) {
            ...
            if (config.dryRun) {
                ...
                logger.info({ committedFiles: config.committedFiles, pr: existingPr.number }, `PR updated`);
            }
            return existingPr;
        }
        ...
        try {
            ...
            if (config.dryRun) {
                ...
                pr = await platform.createPr(branchName, prTitle, prBody, config.labels, false, platformOptions);
            ...
            if (err.statusCode === 502) {
                logger.info({ branch: branchName }, 'Deleting branch due to server error');
                // istanbul ignore if
                if (config.dryRun) {
                    logger.info('DRY-RUN: Would delete branch: ' + config.branchName);
                }
...
async function addAssigneesReviewers(config, pr) {
    if (config.assignees.length > 0) {
        try {
            const assignees = config.assignees.map(assignee => assignee.length && assignee[0] === '@' ? assignee.slice(1) : assignee);
            // istanbul ignore if
            if (config.dryRun) {
            ...
            logger.info({ assignees: config.assignees, err }, 'Failed to add assignees');
        }
    }
    if (config.reviewers.length > 0) {
        try {
            const reviewers = config.reviewers.map(reviewer => reviewer.length && reviewer[0] === '@' ? reviewer.slice(1) : reviewer);
            // istanbul ignore if
            if (config.dryRun) {
                logger.info('DRY-RUN: Would add assignees to PR #', pr.number);
            }
            ...
            logger.info({ assignees: config.assignees, err }, 'Failed to add reviewers');
        }
    }
}
async function checkAutoMerge(pr, config) {
    logger.trace({ config }, 'checkAutoMerge');
    const { branchName, automerge, automergeType, automergeComment, requiredStatusChecks, } = config;
    logger.debug({ automerge, automergeType, automergeComment }, `Checking #${pr.number} for automerge`);
    if (automerge) {
        ...
        const branchStatus = await platform.getBranchStatus(branchName, requiredStatusChecks);
        if (branchStatus !== 'success') {
            logger.info(`PR is not ready for merge (branch status is ${branchStatus})`);
            return false;
        }
