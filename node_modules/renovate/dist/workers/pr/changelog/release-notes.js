...
async function getReleaseNotes(repository, version, githubBaseURL, githubApiBaseURL) {
...
async function getReleaseNotesMd(repository, version, githubBaseURL, githubApiBaseUrl) {
    logger.trace(`getReleaseNotes(${repository}, ${version})`);
    ...
    try {
        ...
        /* istanbul ignore if */
        if (files.length > 1) {
            logger.info(`Multiple candidates for changelog file, using ${changelogFile}`);
        }
...
async function addReleaseNotes(input) {
    ...
    for (const v of input.versions) {
        ...
        if (!releaseNotes) {
            releaseNotes = await getReleaseNotesMd(repository, v.version, input.project.githubBaseURL, input.project.githubApiBaseURL);
            if (!releaseNotes) {
                logger.trace('No markdown release notes found for v' + v.version);
                releaseNotes = await getReleaseNotes(repository, v.version, input.project.githubBaseURL, input.project.githubApiBaseURL);
            }
            ...
            await renovateCache.set(cacheNamespace, cacheKey, releaseNotes, cacheMinutes);
        }
