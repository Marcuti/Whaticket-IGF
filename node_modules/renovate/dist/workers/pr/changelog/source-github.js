...
async function getChangeLogJSON({ versionScheme, fromVersion, toVersion, sourceUrl, releases, depName, manager, }) {
    if (sourceUrl === 'https://github.com/DefinitelyTyped/DefinitelyTyped') {
        logger.debug('No release notes for @types');
        return null;
    }
    const { isVersion, equals, isGreaterThan, sortVersions } = versioning.get(versionScheme);
    ...
    // compare versions
    const include = version => isGreaterThan(version, fromVersion) && !isGreaterThan(version, toVersion);
    for (let i = 1; i < validReleases.length; i += 1) {
        const prev = validReleases[i - 1];
        const next = validReleases[i];
        if (include(next.version)) {
            let release = await renovateCache.get(cacheNamespace, getCacheKey(prev.version, next.version));
            if (!release) {
                release = {
                ...
                await renovateCache.set(cacheNamespace, getCacheKey(prev.version, next.version), release, cacheMinutes);
            }
