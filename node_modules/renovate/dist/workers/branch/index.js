...
async function processBranch(branchConfig, prHourlyLimitReached, packageFiles) {
    ...
    try {
        logger.debug(`Branch has ${dependencies ? dependencies.length : 0} upgrade(s)`);
        // Check if branch already existed
        const existingPr = branchPr ? undefined : await prAlreadyExisted(config);
        if (existingPr && !masterIssueCheck) {
            logger.debug({ prTitle: config.prTitle }, 'Closed PR already exists. Skipping branch.');
            if (existingPr.state === 'closed') {
                ...
                if (!config.suppressNotifications.includes('prIgnoreNotification')) {
                    // istanbul ignore if
                    if (config.dryRun) {
                        logger.info('DRY-RUN: Would ensure closed PR comment in PR #' +
                            existingPr.number);
                    }
                    else {
                ...
                logger.info({ pr: existingPr.number }, 'Merged PR is blocking this branch');
            }
        ...
        if (branchExists) {
            ...
            if (branchPr) {
                ...
                if (branchPr.state !== 'open') {
                    logger.info('PR has been closed or merged since this run started - aborting');
                ...
                if (!branchPr.canRebase) {
                    ...
                    const prRebaseChecked = branchPr.body &&
                        branchPr.body.includes(`- [x] <!-- ${appSlug}-rebase -->`);
                    if (prRebaseChecked || titleRebase || labelRebase) {
                        // istanbul ignore if
                        if (config.dryRun) {
                            logger.info('DRY-RUN: Would ensure PR edited comment removal in PR #' +
                                branchPr.number);
                        }
                        else {
        ...
        if (!config.isScheduledNow && !masterIssueCheck) {
            ...
            const prRebaseChecked = branchPr &&
                branchPr.body &&
                branchPr.body.includes(`- [x] <!-- ${appSlug}-rebase -->`);
        ...
        if (config.updateType !== 'lockFileMaintenance' &&
            ...
            logger.info('Skipping branch creation due to unpublishSafe + status checks');
            return 'pending';
        }
        // istanbul ignore if
        ...
        if (config.artifactErrors && config.artifactErrors.length) {
            if (config.releaseTimestamp) {
                ...
                if (releaseTimestamp.plus({ days: 1 }) < DateTime.local()) {
                    ...
                    logger.info('PR is less than a day old but branchExists so updating anyway');
                }
                else {
        ...
        if (config.updateType === 'lockFileMaintenance' &&
            ...
            !config.parentBranch &&
            branchExists) {
            logger.info('Deleting lock file maintenance branch as master lock file no longer needs updating');
            // istanbul ignore if
            if (config.dryRun) {
        ...
        if (pr) {
            ...
            if (config.artifactErrors && config.artifactErrors.length) {
                ...
                if (!(config.suppressNotifications.includes('artifactErrors') ||
                    ...
                    // istanbul ignore if
                    if (config.dryRun) {
                        logger.info('DRY-RUN: Would ensure lock file error comment in PR #' +
                            pr.number);
                    }
                    else {
                        await platform.ensureComment(pr.number, topic, content);
                        // TODO: remoe this soon once they're all cleared out
                        await platform.ensureCommentRemoval(pr.number, ':warning: Lock file problem');
                    }
                }
                const context = `${appSlug}/artifacts`;
                const description = 'Artifact file update failure';
                const state = 'failure';
                const existingState = await platform.getBranchStatusCheck(config.branchName, context);
                ...
                if (existingState !== state) {
                    ...
                    if (config.dryRun) {
                        ...
                        await platform.setBranchStatus(config.branchName, context, description, state);
                    }
                }
            }
            else {
                if (config.updatedArtifacts && config.updatedArtifacts.length) {
                    // istanbul ignore if
                    if (config.dryRun) {
                        logger.info('DRY-RUN: Would ensure comment removal in PR #' + pr.number);
                    }
                    else {
