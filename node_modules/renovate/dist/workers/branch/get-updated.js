...
async function getUpdatedPackageFiles(config) {
    ...
    for (const upgrade of config.upgrades) {
        ...
        if (upgrade.updateType !== 'lockFileMaintenance') {
            const existingContent = updatedFileContents[packageFile] ||
            ...
            if (!newContent) {
                ...
                logger.debug({ existingContent, config: upgrade }, 'Error updating file');
    ...
    for (const packageFile of updatedPackageFiles) {
        ...
        if (getArtifacts) {
            const results = await getArtifacts(packageFile.name, updatedDeps, packageFile.contents, config);
