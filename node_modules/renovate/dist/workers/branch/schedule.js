...
function hasValidSchedule(schedule) {
    ...
    const hasFailedSchedules = schedule.some(scheduleText => {
        ...
        if (!parsedSchedule.schedules.some(s => s.d !== undefined || s.D || s.t_a !== undefined || s.t_b)) {
...
function isScheduledNow(config) {
    ...
    if (!is.array(configSchedule)) {
        logger.warn(`config schedule is not an array: ${JSON.stringify(configSchedule)}`);
        ...
        return parsedSchedule.schedules.some(schedule => {
            ...
            if (schedule.d) {
                ...
                if (!scheduledDays.includes(currentDay)) {
                    logger.debug(`Does not match schedule because ${currentDay} is not in ${scheduledDays}`);
                    return false;
                }
            }
            if (schedule.D) {
            ...
            if (schedule.t_a) {
                ...
                if (currentSeconds < startSeconds) {
                    logger.debug(`Does not match schedule because ${currentSeconds} is earlier than ${startSeconds}`);
                    return false;
                }
            }
            // Check for end time
            if (schedule.t_b) {
                ...
                if (currentSeconds > endSeconds) {
                    logger.debug(`Does not match schedule because ${currentSeconds} is later than ${endSeconds}`);
                    return false;
                }
            }
