...
async function commitFilesToBranch(config) {
    ...
    if (is.nonEmptyArray(config.excludeCommitPaths)) {
        updatedFiles = updatedFiles.filter(f => {
            ...
            const matchesExcludePaths = config.excludeCommitPaths.some(path => minimatch(filename, path, { dot: true }));
    ...
    if (is.nonEmptyArray(updatedFiles)) {
        ...
        if (config.dryRun) {
            ...
            const res = await platform.commitFilesToBranch(config.branchName, updatedFiles, config.commitMessage, config.baseBranch || undefined);
            if (res) {
