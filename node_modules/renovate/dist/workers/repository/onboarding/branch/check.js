"use strict";
const { appName, appSlug, configFileNames, onboardingBranch, onboardingPrTitle, } = require('../../../../config/app-strings');
...
const closedPrExists = () => platform.findPr(onboardingBranch, onboardingPrTitle, '!open');
const isOnboarded = async (config) => {
    ...
    if (!config.suppressNotifications.includes('onboardingClose')) {
        // ensure PR comment
        await platform.ensureComment(pr.number, `${appName} is disabled`, `${appName} is disabled due to lack of config. If you wish to reenable it, you can either (a) commit a config file to your base branch, or (b) rename this closed PR to trigger a replacement onboarding PR.`);
    }
    throw new Error('disabled');
};
