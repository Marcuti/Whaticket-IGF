...
async function getRenovateFiles(prNo) {
    return (await platform.getPrFiles(prNo)).filter(file => configFileNames.includes(file));
}
async function validatePrs(config) {
    ...
    for (const pr of renovatePrs) {
        try {
            ...
            logger.info({ prNo: pr.number, title: pr.title, renovateFiles }, 'PR has renovate files');
            for (const file of renovateFiles) {
                ...
                if (parsed) {
                    const toValidate = file === 'package.json'
                    ...
                    if (toValidate) {
                        ...
                        if (errors && errors.length) {
                            validations = validations.concat(errors.map(error => ({
                                file,
            ...
            if (pr.sourceRepo === config.repository) {
                ...
                await platform.setBranchStatus(pr.branchName, context, description, status);
            }
            else {
