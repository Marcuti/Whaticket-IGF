"use strict";
const { appName, onboardingBranch, onboardingPrTitle, } = require('../../config/app-strings');
module.exports = {
...
async function raiseConfigWarningIssue(config, error) {
    ...
    if (pr && pr.state && pr.state.startsWith('open')) {
        ...
        const res = await platform.ensureIssue(`Action Required: Fix ${appName} Configuration`, body);
        if (res === 'created') {
