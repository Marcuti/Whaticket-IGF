...
function branchifyUpgrades(config, packageFiles) {
    ...
    logger.debug(`${updates.length} flattened updates found: ${updates
        .map(u => u.depName)
        .join(', ')}`);
    const errors = [];
    const warnings = [];
    ...
    for (const u of updates) {
        ...
        if (update.groupName) {
            ...
            logger.debug(`Dependency ${update.depName} is part of group ${update.groupName}`);
            ...
            update.branchName = handlebars.compile(update.group.branchName || update.branchName)(update);
        }
        else {
        ...
        update.branchName = handlebars.compile(update.branchName)(update);
        update.branchName = cleanBranchName(handlebars.compile(update.branchName)(update));
    ...
    try {
        ...
        for (const [key, value] of Object.entries(branchUpdates)) {
            if (Object.keys(value).length > 1) {
                ...
                logger.info({ sourceUrl, toVersion, branches: value }, 'Found sourceUrl with multiple branches that should probably be combined into a group');
            }
        }
    }
    catch (err) {
