...
function getIncludedFiles(fileList, includePaths) {
    ...
    return fileList.filter(file => includePaths.some(includePath => file === includePath || minimatch(file, includePath, { dot: true })));
}
...
function getMatchingFiles(fileList, manager, fileMatch) {
    ...
    for (const match of fileMatch) {
        ...
        matchedFiles = matchedFiles.concat(fileList.filter(file => file.match(new RegExp(match))));
    }
