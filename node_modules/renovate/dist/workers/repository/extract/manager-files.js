...
const { extractAllPackageFiles, extractPackageFile, get, } = require('../../../manager');
const { getIncludedFiles, filterIgnoredFiles, getMatchingFiles, } = require('./file-match');
async function getManagerPackageFiles(config, managerConfig) {
    ...
    if (config.enabledManagers.length &&
    ...
    const matchedFiles = getMatchingFiles(fileList, manager, config[manager].fileMatch);
    // istanbul ignore else
    if (matchedFiles.length) {
        logger.debug(`Matched ${matchedFiles.length} file(s) for manager ${manager}: ${matchedFiles.join(', ')}`);
    }
    else {
        return [];
    }
