...
async function ensureMasterIssue(config, branches) {
    if (!(config.masterIssue || branches.some(branch => branch.masterIssueApproval))) {
        return;
    }
    ...
    if (!branches.length ||
        ...
        await platform.ensureIssue(config.masterIssueTitle, 'This repository is up-to-date and has no outstanding updates open or pending.');
        return;
    }
    ...
    const rateLimited = branches.filter(branch => branch.res && branch.res.endsWith('pr-hourly-limit-reached'));
    ...
    if (alreadyExisted.length) {
        ...
        for (const branch of alreadyExisted) {
            const pr = await platform.findPr(branch.branchName, branch.prTitle, '!open');
