...
async function mergeRenovateConfig(config) {
    ...
    if (configFile === 'package.json') {
        ...
        if (fileType === '.json5') {
            try {
                ...
                logger.debug({ renovateConfig }, 'Error parsing renovate config renovate.json5');
            ...
            let jsonValidationError = jsonValidator.validate(renovateConfig, allowDuplicateKeys);
            ...
            jsonValidationError = jsonValidator.validate(renovateConfig, allowDuplicateKeys);
            if (jsonValidationError) {
                const error = new Error('config-validation');
                error.configFile = configFile;
    ...
    const resolvedConfig = decryptConfig(await presets.resolveConfigPresets(decryptedConfig), config.privateKey);
    delete resolvedConfig.privateKey;
    logger.trace({ config: resolvedConfig }, 'resolved config');
    // istanbul ignore if
    if (resolvedConfig.npmrc) {
        logger.debug('Ignoring any .npmrc files in repository due to configured npmrc');
    ...
    if (returnConfig.ignorePaths && returnConfig.ignorePaths.length) {
        logger.debug({ ignorePaths: returnConfig.ignorePaths }, `Found repo ignorePaths`);
    }
    return returnConfig;
}
module.exports = {
