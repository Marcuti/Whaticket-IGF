...
async function getPrHourlyRemaining(config) {
    if (config.prHourlyLimit) {
        ...
        try {
            const soFarThisHour = prList.filter(pr => pr.branchName !== onboardingBranch &&
...
async function getPrsRemaining(config, branches) {
    ...
    const concurrentRemaining = await module.exports.getConcurrentPrsRemaining(config, branches);
