...
const { getPkgReleases, supportsDigests, getDigest, } = require('../../../../datasource');
...
async function lookupUpdates(config) {
    ...
    const { equals, getMajor, getMinor, isGreaterThan, isSingleVersion, isCompatible, isValid, isVersion, matches, getNewValue, valueToVersion, } = versioning.get(config.versionScheme);
    ...
    if (isValid(currentValue)) {
        ...
        if (!dependency) {
            ...
            logger.info({ dependency: depName, packageFile: config.packageFile }, result.message);
        ...
        if (config.followTag) {
            ...
            allVersions = allVersions.filter(v => v === taggedVersion ||
                (v === currentValue && isGreaterThan(taggedVersion, currentValue)));
        }
        // Check that existing constraint can be satisfied
        const allSatisfyingVersions = allVersions.filter(version => matches(version, currentValue));
        ...
        if (fromVersion &&
            ...
            res.updates.push({
                updateType: 'pin',
                isPin: true,
                newValue: getNewValue(currentValue, rangeStrategy, fromVersion, fromVersion),
        ...
        const filteredVersions = filterVersions(config, filterStart, dependency.latestVersion, allVersions, releases).filter(version => 
        ...
        for (const toVersion of filteredVersions) {
            ...
            try {
                update.newValue = getNewValue(currentValue, rangeStrategy, fromVersion, toVersion);
            }
            catch (err) /* istanbul ignore next */ {
                logger.warn({ err, currentValue, rangeStrategy, fromVersion, toVersion }, 'getNewValue error');
            ...
            if (!update.newValue || update.newValue === currentValue) {
                ...
                if (rangeStrategy === 'bump') {
                    logger.trace({ depName, currentValue, lockedVersion, toVersion }, 'Skipping bump because newValue is the same');
                    continue; // eslint-disable-line no-continue
                }
            ...
            const updateRelease = releases.find(release => equals(release.version, toVersion));
        ...
        res.releases = releases.filter(release => filteredVersions.length &&
...
function getFromVersion(config, rangeStrategy, allVersions) {
    ...
    const { isSingleVersion, isVersion, maxSatisfyingVersion, minSatisfyingVersion, } = versioning.get(versionScheme);
