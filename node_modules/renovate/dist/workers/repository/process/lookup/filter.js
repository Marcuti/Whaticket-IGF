...
function filterVersions(config, fromVersion, latestVersion, versions, releases) {
    const { versionScheme, ignoreUnstable, ignoreDeprecated, respectLatest, allowedVersions, } = config;
    const { getMajor, getMinor, getPatch, isGreaterThan, isStable, isValid, matches, } = versioning.get(versionScheme);
    if (!fromVersion) {
        return [];
    }
    // Leave only versions greater than current
    let filteredVersions = versions.filter(version => isGreaterThan(version, fromVersion));
    ...
    if (ignoreDeprecated && fromRelease && !fromRelease.isDeprecated) {
        filteredVersions = filteredVersions.filter(version => {
            const versionRelease = releases.find(release => release.version === version);
            if (versionRelease.isDeprecated) {
                logger.debug(`Skipping ${config.depName}@${version} because it is deprecated`);
                return false;
            }
            return true;
        });
    ...
    if (allowedVersions) {
        if (isValid(allowedVersions)) {
            filteredVersions = filteredVersions.filter(version => matches(version, allowedVersions));
        }
            ...
            filteredVersions = filteredVersions.filter(version => semver.satisfies(semver.coerce(version), allowedVersions));
        }
        else {
            logger.warn({ depName: config.depName }, `Invalid allowedVersions: "${allowedVersions}"`);
        }
    }
    ...
    if (!isStable(fromVersion)) {
        ...
        return filteredVersions.filter(version => isStable(version) ||
    ...
    return filteredVersions.filter(version => !isGreaterThan(version, latestVersion));
}
