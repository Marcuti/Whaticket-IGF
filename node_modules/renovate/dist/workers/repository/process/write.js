...
async function writeUpdates(config, packageFiles, allBranches) {
    ...
    logger.info(`Processing ${branches.length} branch${branches.length !== 1 ? 'es' : ''}: ${branches
        .map(b => b.branchName)
        .sort()
        .join(', ')}`);
    ...
    for (const branch of branches) {
        const res = await branchWorker.processBranch(branch, prsRemaining <= 0, packageFiles);
        branch.res = res;
