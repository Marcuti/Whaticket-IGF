...
async function fetchDepUpdates(packageFileConfig, dep) {
    ...
    if (depConfig.ignoreDeps.includes(depName)) {
    ...
    else if (depConfig.internalPackages &&
        depConfig.internalPackages.includes(depName)) {
        logger.debug({ dependency: dep.depName }, 'Dependency is ignored due to being internal');
        ...
        if (dep.updates.length) {
            logger.trace({ dependency: depName }, `${dep.updates.length} result(s): ${dep.updates.map(upgrade => upgrade.newValue)}`);
        }
        logger.trace({
            packageFile,
            manager,
...
async function fetchUpdates(config, packageFiles) {
    ...
    const allManagerJobs = Object.keys(packageFiles).map(manager => fetchManagerUpdates(config, packageFiles, manager));
