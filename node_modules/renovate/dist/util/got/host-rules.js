    ...
    handler: (options, next) => {
        ...
        if (!options.headers.authorization) {
            if (hostRule.password) {
                logger.debug('Applying Basic authentication for host ' + options.hostname);
                const auth = Buffer.from(`${hostRule.username || ''}:${hostRule.password}`).toString('base64');
                opts.headers.authorization = `Basic ${auth}`;
            }
            else if (hostRule.token) {
                logger.debug('Applying Bearer authentication for host ' + options.hostname);
