...
function applyPackageRules(inputConfig) {
    ...
    const { versionScheme, packageFile, depType, depTypes, depName, currentValue, fromVersion, lockedVersion, updateType, isBump, sourceUrl, language, baseBranch, manager, datasource, } = config;
    const packageRules = config.packageRules || [];
    logger.trace({ dependency: depName, packageRules }, `Checking against ${packageRules.length} packageRules`);
    packageRules.forEach(packageRule => {
        ...
        if (packageNames.length || packagePatterns.length) {
            ...
            if (!isMatch) {
                for (const packagePattern of packagePatterns) {
                    const packageRegex = new RegExp(packagePattern === '^*$' || packagePattern === '*'
                        ? '.*'
                        : packagePattern);
        ...
        if (excludePackagePatterns.length) {
            ...
            for (const pattern of excludePackagePatterns) {
                const packageRegex = new RegExp(pattern === '^*$' || pattern === '*' ? '.*' : pattern);
                if (depName && depName.match(packageRegex)) {
                    logger.trace(`${depName} matches against ${packageRegex}`);
                    isMatch = true;
                }
        ...
        if (sourceUrlPrefixes.length) {
            const isMatch = sourceUrlPrefixes.some(prefix => sourceUrl && sourceUrl.startsWith(prefix));
            positiveMatch = positiveMatch || isMatch;
            negativeMatch = negativeMatch || !isMatch;
        }
