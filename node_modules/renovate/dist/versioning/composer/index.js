...
const isGreaterThan = (a, b) => npm.isGreaterThan(composer2npm(a), composer2npm(b));
const isLessThanRange = (version, range) => npm.isLessThanRange(composer2npm(version), composer2npm(range));
...
const matches = (version, range) => npm.matches(composer2npm(version), composer2npm(range));
const maxSatisfyingVersion = (versions, range) => npm.maxSatisfyingVersion(versions.map(composer2npm), composer2npm(range));
const minSatisfyingVersion = (versions, range) => npm.minSatisfyingVersion(versions.map(composer2npm), composer2npm(range));
function getNewValue(currentValue, rangeStrategy, fromVersion, toVersion) {
    ...
    else if (npm.isVersion(padZeroes(toVersion)) &&
        npm.isValid(currentValue) &&
        composer2npm(currentValue) === currentValue) {
        newValue = npm.getNewValue(currentValue, rangeStrategy, fromVersion, padZeroes(toVersion));
    }
    ...
    if (currentValue.includes(' || ')) {
        const lastValue = currentValue
            ...
            .pop()
            .trim();
        const replacementValue = getNewValue(lastValue, rangeStrategy, fromVersion, toVersion);
    ...
    if (!newValue) {
        logger.warn({ currentValue, rangeStrategy, fromVersion, toVersion }, 'Unsupported composer value');
        newValue = toVersion;
    }
