...
function getNewValue(currentValue, rangeStrategy, fromVersion, toVersion) {
    ...
    if (!['replace', 'bump'].includes(rangeStrategy)) {
        logger.info('Unsupported rangeStrategy: ' +
            rangeStrategy +
    ...
    const result = ranges
        ...
        logger.error({ toVersion, currentValue, range }, 'pep440: failed to process range');
        // istanbul ignore next
        return null;
    })
        .filter(Boolean)
    ...
    if (!satisfies(toVersion, result)) {
        ...
        logger.error({ result, toVersion, currentValue }, 'pep440: failed to calcuate newValue');
        return null;
    }
    return result;
}
