...
async function getPkgReleases({ lookupName, registryUrls }) {
    ...
    const repositories = registryUrls.map(repository => repository.replace(/\/?$/, '/'));
    logger.debug(`Found ${repositories.length} repositories for ${dependency.display}`);
    const repoForVersions = {};
    for (let i = 0; i < repositories.length; i += 1) {
        const repoUrl = repositories[i];
        logger.debug(`Looking up ${dependency.display} in repository #${i} - ${repoUrl}`);
        const mavenMetadata = await downloadMavenXml(dependency, repoUrl, 'maven-metadata.xml');
        if (mavenMetadata) {
            const newVersions = extractVersions(mavenMetadata).filter(version => !versions.includes(version));
    ...
    const dependencyInfo = await getDependencyInfo(dependency, repoUrl, latestVersion);
    return {
        ...dependency,
        ...dependencyInfo,
...
async function downloadMavenXml(dependency, repoUrl, dependencyFilePath) {
    const pkgUrl = new url.URL(`${dependency.dependencyUrl}/${dependencyFilePath}`, repoUrl);
    let rawContent;
    switch (pkgUrl.protocol) {
        ...
        case 's3:':
            ...
            return null;
        default:
            logger.warn(`Invalid protocol ${pkgUrl.protocol} in repository ${repoUrl}`);
            return null;
    }
    if (!rawContent) {
...
async function downloadHttpProtocol(pkgUrl) {
    ...
    try {
        ...
        if (isNotFoundError(err)) {
            ...
            logger.warn({ pkgUrl }, 'Dependency lookup unauthorized. Please add authentication with a hostRule');
        }
...
function getLatestVersion(versions) {
    if (versions.length === 0)
        return null;
    return versions.reduce((latestVersion, version) => compare(version, latestVersion) === 1 ? version : latestVersion);
}
