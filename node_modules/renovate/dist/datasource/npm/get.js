...
async function getDependency(name) {
    ...
    if (authInfo && authInfo.type && authInfo.token) {
        ...
        logger.trace({ token: maskToken(authInfo.token), npmName: name }, 'Using auth for npm lookup');
    }
    else if (process.env.NPM_TOKEN && process.env.NPM_TOKEN !== 'undefined') {
        headers.authorization = `Bearer ${process.env.NPM_TOKEN}`;
    }
    if (pkgUrl.startsWith('https://registry.npmjs.org') &&
    ...
    try {
        ...
        if (returnedName.toLowerCase() !== name.toLowerCase()) {
            logger.warn({ lookupName: name, returnedName: res.name, regUrl }, 'Returned name does not match with requested name');
            return null;
        }
        ...
        if (res.repository && res.repository.url) {
            ...
            res.repository.url = res.repository.url.replace('www.github.com', 'github.com');
