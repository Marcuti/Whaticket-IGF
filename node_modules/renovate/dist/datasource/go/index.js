...
async function getDatasource(name) {
    ...
    try {
        ...
        const sourceMatch = res.match(new RegExp(`<meta name="go-source" content="${name}\\s+([^\\s]+)`));
        if (sourceMatch) {
...
async function getPkgReleases({ lookupName }) {
    ...
    if (source && source.datasource === 'github') {
        ...
        if (res && res.releases) {
            res.releases = res.releases.filter(release => release.version && release.version.startsWith('v'));
        }
        return res;
    }
    return null;
