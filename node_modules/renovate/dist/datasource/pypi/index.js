...
async function getDependency(depName, hostUrl, compatibility) {
    ...
    try {
        ...
        if (!(dep.info && normalizeName(dep.info.name) === normalizeName(depName))) {
            logger.warn({ lookupUrl, lookupName: depName, returnedName: dep.info.name }, 'Returned name does not match with requested name');
            return null;
        }
        ...
        logger.info('pypi dependency not found: ' + depName + '(searching in ' + hostUrl + ')');
        return null;
    }
}
