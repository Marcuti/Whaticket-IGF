    ...
    try {
        ...
        do {
            ...
            if (pkgVersionsListRaw.statusCode !== 200) {
                logger.debug({ dependency: pkgName, pkgVersionsListRaw }, `nuget registry failure: status code != 200`);
                return null;
            }
            const pkgVersionsListDoc = new XmlDocument(pkgVersionsListRaw.body);
            const pkgInfoList = pkgVersionsListDoc.children.filter(node => node.name === 'entry');
            ...
            const nextPkgUrlListLink = pkgVersionsListDoc.children.find(node => node.name === 'link' && node.attr.rel === 'next');
