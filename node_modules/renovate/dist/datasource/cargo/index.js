...
async function getPkgReleases({ lookupName }) {
    ...
    try {
        ...
        if (!res || !res.body) {
            logger.warn({ dependency: lookupName }, `Received invalid crate data from ${crateUrl}`);
            return null;
        }
        res = res.body;
        res = res.split('\n');
        ...
        if (res.length === 0) {
            logger.warn({ dependency: lookupName }, `Received empty list from ${crateUrl}`);
            return null;
        }
        // Filter empty lines (takes care of trailing \n)
        res = res.map(JSON.parse);
        if (res[0].name !== lookupName) {
            logger.warn({ dependency: lookupName }, `Received invalid crate name from ${crateUrl}`);
            return null;
        }
        if (!res[0].vers) {
            logger.warn({ dependency: lookupName }, `Recieved invalid data (vers field doesn't exist) from ${crateUrl}`);
            return null;
        }
        const result = {
            releases: [],
        ...
        logger.warn({ err, lookupName }, 'cargo crates.io lookup failure: Unknown error');
        return null;
    }
}
