...
async function getTagCommit(githubRepo, tag) {
    const cachedResult = await renovateCache.get(cacheNamespace, getCacheKey(githubRepo, `tag-${tag}`));
    // istanbul ignore if
    if (cachedResult) {
        return cachedResult;
    }
    ...
    try {
        ...
        logger.info({ githubRepo, err }, 'Error getting tag commit from GitHub repo');
    }
    if (!digest) {
        return null;
    }
    const cacheMinutes = 120;
    await renovateCache.set(cacheNamespace, getCacheKey(githubRepo, `tag-${tag}`), digest, cacheMinutes);
    return digest;
}
async function getDigest({ lookupName: githubRepo }, newValue) {
    ...
    const cachedResult = await renovateCache.get(cacheNamespace, getCacheKey(githubRepo, 'commit'));
    // istanbul ignore if
    if (cachedResult) {
        return cachedResult;
    }
    ...
    try {
        ...
        logger.info({ githubRepo, err }, 'Error getting latest commit from GitHub repo');
    }
    if (!digest) {
        return null;
    }
    const cacheMinutes = 10;
    await renovateCache.set(cacheNamespace, getCacheKey(githubRepo, 'commit'), digest, cacheMinutes);
    return digest;
}
/*
...
async function getPkgReleases({ lookupName: repo, lookupType }) {
    let versions;
    const cachedResult = await renovateCache.get(cacheNamespace, getCacheKey(repo, lookupType || 'tags'));
    // istanbul ignore if
    if (cachedResult) {
        return cachedResult;
    }
    try {
        if (lookupType === 'releases') {
            ...
            versions = (await ghGot(url, { paginate: true })).body.map(o => o.tag_name);
        }
        else {
            // tag
    ...
    const cacheMinutes = 10;
    await renovateCache.set(cacheNamespace, getCacheKey(repo, lookupType), dependency, cacheMinutes);
    return dependency;
}
