...
class Storage {
    ...
    async _resetToBranch(branchName) {
    ...
    async initRepo(args) {
        ...
        if (await fs_extra_1.default.exists(gitHead)) {
            try {
                this._git = promise_1.default(cwd).silent(true);
                ...
                const fetchSeconds = Math.round(1 + 10 * convert_hrtime_1.default(process.hrtime(fetchStart)).seconds) / 10;
        ...
        if (clone) {
            await fs_extra_1.default.emptyDir(cwd);
            this._git = promise_1.default(cwd).silent(true);
            ...
            const cloneSeconds = Math.round(1 + 10 * convert_hrtime_1.default(process.hrtime(cloneStart)).seconds) /
                10;
    ...
    async setBaseBranch(branchName) {
        if (branchName) {
            if (!(await this.branchExists(branchName))) {
                throw new Error('Cannot set baseBranch to something that does not exist');
            }
     ...
     */
    async setBranchPrefix(branchPrefix) {
    ...
    async _deleteLocalBranch(branchName) {
    ...
    async deleteBranch(branchName) {
        try {
            ...
            if (err.message.includes('.github/main.workflow')) {
                logger.warn('A GitHub bug prevents gitFs + GitHub Actions. Please disable gitFs');
            }
            else {
    ...
    async commitFilesToBranch(branchName, files, message, parentBranch = this._config.baseBranch) {
        ...
        try {
            ...
            for (const file of files) {
                ...
                if (file.name === '|delete|') {
                    ...
                    await fs_extra_1.default.outputFile(path_1.join(this._cwd, file.name), Buffer.from(file.contents));
                }
            }
            // istanbul ignore if
