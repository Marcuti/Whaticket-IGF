...
const { appName, appSlug, configFileNames, urls, } = require('../../config/app-strings');
...
async function initRepo({ repository, forkMode, forkToken, gitPrivateKey, gitFs, localDir, includeForks, renovateUsername, }) {
    ...
    try {
        ...
        if (res.body.fork && gitFs && !includeForks) {
            try {
                const renovateConfig = JSON.parse(Buffer.from((await get(`repos/${config.repository}/contents/${defaultConfigFile}`)).body.content, 'base64').toString());
        ...
        if (res.body.full_name && res.body.full_name !== repository) {
            logger.info({ repository, this_repository: res.body.full_name }, 'Repository has been renamed');
    ...
    if (forkMode) {
        ...
        if (existingRepos.includes(config.repository)) {
            logger.info({ repository_fork: config.repository }, 'Found existing fork');
            // Need to update base branch
            logger.debug({ baseBranch: config.baseBranch, parentSha }, 'Setting baseBranch ref in fork');
            // This is a lovely "hack" by GitHub that lets us force update our fork's master
            // with the base commit from the parent repository
            try {
                await get.patch(`repos/${config.repository}/git/refs/heads/${config.baseBranch}`, {
                    body: {
                        sha: parentSha,
                        force: true,
                    },
    ...
    if (config.repoForceRebase === undefined) {
        try {
            ...
            if (branchProtection.required_pull_request_reviews) {
                logger.debug('Branch protection: PR Reviews are required before merging');
                config.prReviewsRequired = true;
            }
            if (branchProtection.required_status_checks) {
                if (branchProtection.required_status_checks.strict) {
                    logger.debug('Branch protection: PRs must be up-to-date before merging');
            ...
            if (err.statusCode === 404) {
                ...
                logger.debug('Branch protection: Do not have permissions to detect branch protection');
            }
            else {
                throw err;
            }
...
async function getBaseCommitSHA() {
    if (!config.baseCommitSHA) {
        config.baseCommitSHA = await config.storage.getBranchCommit(config.baseBranch);
    }
...
async function getBranchProtection(branchName) {
    ...
    const res = await get(`repos/${config.repository}/branches/${branchName}/protection`);
    return res.body;
}
...
function mergeBranch(branchName) {
    ...
    if (config.pushProtection) {
        logger.info({ branch: branchName }, 'Branch protection: Attempting to merge branch when push protection is enabled');
    }
...
async function getBranchStatus(branchName, requiredStatusChecks) {
    ...
    try {
        ...
        if (err.statusCode === 404) {
            logger.info('Received 404 when checking branch status, assuming that branch has been deleted');
            throw new Error('repository-changed');
        }
        logger.info('Unknown error when checking branch status');
        throw err;
    }
    logger.debug({ state: commitStatus.state, statuses: commitStatus.statuses }, 'branch status check result');
    let checkRuns = [];
    if (!config.isGhe) {
        try {
...
async function getIssueList() {
    if (!config.issueList) {
        ...
        const res = await get(`repos/${config.parentRepo || config.repository}/issues?creator=${config.renovateUsername}&state=all&per_page=100&sort=created&direction=asc`, { paginate: 'all', useCache: false });
        // istanbul ignore if
...
async function findIssue(title) {
    ...
    try {
        ...
        const issueBody = (await get(`repos/${config.parentRepo || config.repository}/issues/${issue.number}`)).body.body;
        return {
            number: issue.number,
            body: issueBody,
        };
...
async function ensureIssue(title, body, once = false) {
    ...
    try {
        ...
        if (issues.length) {
            ...
            await get.patch(`repos/${config.parentRepo || config.repository}/issues/${issue.number}`, {
...
async function closeIssue(issueNumber) {
    ...
    await get.patch(`repos/${config.parentRepo || config.repository}/issues/${issueNumber}`, {
        body: { state: 'closed' },
    });
}
...
async function addReviewers(prNo, reviewers) {
    ...
    await get.post(`repos/${config.parentRepo ||
...
async function editComment(commentId, body) {
    ...
    await get.patch(`repos/${config.parentRepo ||
...
async function deleteComment(commentId) {
    ...
    await get.delete(`repos/${config.parentRepo ||
        config.repository}/issues/comments/${commentId}`);
}
async function ensureComment(issueNo, topic, content) {
    try {
        ...
        if (err.message === 'Unable to create comment because issue is locked. (403)') {
...
async function getPrList() {
    ...
    if (!config.prList) {
        ...
        const res = await get(`repos/${config.parentRepo ||
            config.repository}/pulls?per_page=100&state=all`, { paginate: true });
        config.prList = res.body.map(pr => ({
            number: pr.number,
            branchName: pr.head.ref,
            sha: pr.head.sha,
            title: pr.title,
            state: pr.state === 'closed' && pr.merged_at && pr.merged_at.length
                ? 'merged'
                : pr.state,
...
async function createPr(branchName, title, body, labels, useDefaultBranch, platformOptions = {}) {
    ...
    const pr = (await get.post(`repos/${config.parentRepo || config.repository}/pulls`, options)).body;
    ...
    if (platformOptions.statusCheckVerify) {
        ...
        await setBranchStatus(branchName, `${appSlug}/verify`, `${appName} verified pull request`, 'success', urls.homepage);
    }
    return pr;
}
async function getOpenPrs() {
    // istanbul ignore if
    if (config.isGhe) {
        logger.debug('Skipping unsupported graphql PullRequests.mergeStateStatus query on GHE');
        return {};
    }
    if (!config.openPrList) {
      ...
      `;
            ...
            for (const pr of res.data.repository.pullRequests.nodes) {
                ...
                const hasNegativeReview = pr.reviews && pr.reviews.nodes && pr.reviews.nodes.length > 0;
                pr.canMerge =
                ...
                if (pr.mergeStateStatus === 'BEHIND') {
                    ...
                    if (pr.commits.nodes[0].commit.parents.edges.length &&
...
async function getClosedPrs() {
    ...
    if (!config.closedPrList) {
      ...
      `;
            ...
            // istanbul ignore if
            if (!res.data) {
                logger.info({ query, res }, 'No graphql res.data, returning empty list');
                return {};
            }
            for (const pr of res.data.repository.pullRequests.nodes) {
                // https://developer.github.com/v4/object/pullrequest/
...
async function getPr(prNo) {
    ...
    logger.info({ prNo }, 'PR not found in open or closed PRs list - trying to fetch it directly');
    const pr = (await get(`repos/${config.parentRepo || config.repository}/pulls/${prNo}`)).body;
    if (!pr) {
        return null;
    }
    // Harmonise PR values
    ...
    if (pr.state === 'open') {
        ...
        if (pr.commits === 1) {
            if (global.gitAuthor) {
                // Check against gitAuthor
                const commitAuthorEmail = (await get(`repos/${config.parentRepo ||
                    config.repository}/pulls/${prNo}/commits`)).body[0].commit.author.email;
                if (commitAuthorEmail === global.gitAuthor.email) {
                    logger.debug({ prNo }, '1 commit matches configured gitAuthor so can rebase');
                    pr.canRebase = true;
                }
                else {
                    logger.debug({
                ...
                logger.debug({ prNo }, '1 commit and no configured gitAuthor so can rebase');
                pr.canRebase = true;
            }
        }
        else {
            ...
            const prCommits = (await get(`repos/${config.parentRepo || config.repository}/pulls/${prNo}/commits`)).body;
            // Filter out "Update branch" presses
            const remainingCommits = prCommits.filter(commit => {
                const isWebflow = commit.committer && commit.committer.login === 'web-flow';
                if (!isWebflow) {
                    // Not a web UI commit, so keep it
                    return true;
                }
                const isUpdateBranch = commit.commit &&
...
async function getPrFiles(prNo) {
    ...
    const files = (await get(`repos/${config.parentRepo || config.repository}/pulls/${prNo}/files`)).body;
...
async function updatePr(prNo, title, body) {
    ...
    try {
        await get.patch(`repos/${config.parentRepo || config.repository}/pulls/${prNo}`, options);
...
async function mergePr(prNo, branchName) {
    ...
    // istanbul ignore if
    if (config.pushProtection) {
        logger.info({ branch: branchName, prNo }, 'Branch protection: Cannot automerge PR when push protection is enabled');
        return false;
    }
    // istanbul ignore if
    if (config.prReviewsRequired) {
        logger.debug({ branch: branchName, prNo }, 'Branch protection: Attempting to merge PR when PR reviews are enabled');
        const repository = config.parentRepo || config.repository;
        ...
        if (!isApproved) {
            logger.info({ branch: branchName, prNo }, 'Branch protection: Cannot automerge PR until there is an approving review');
            return false;
        }
    ...
    if (config.mergeMethod) {
        ...
        try {
            ...
            if (err.statusCode === 405) {
                // istanbul ignore next
                logger.info({ response: err.response ? err.response.body : undefined }, 'GitHub blocking PR merge -- will keep trying');
            }
            else {
    ...
    if (!automerged) {
        ...
        try {
            ...
            try {
                ...
                logger.debug({ err: err2 }, `Failed to ${options.body.merge_method} PR`);
                try {
                    ...
                    logger.debug({ err: err3 }, `Failed to ${options.body.merge_method} PR`);
...
function smartTruncate(input) {
    ...
    const releaseNotesMatch = input.match(new RegExp(`### Release Notes.*### ${appName} configuration`, 'ms'));
    // istanbul ignore if
...
async function getVulnerabilityAlerts() {
    // istanbul ignore if
    if (config.isGhe) {
        logger.debug('Skipping unsupported graphql vulnerabilityAlerts query on GHE');
        return [];
    }
    const headers = {
  ...
  query {
    ...
    try {
        ...
        if (res.data.repository.vulnerabilityAlerts) {
            alerts = res.data.repository.vulnerabilityAlerts.edges.map(edge => edge.node);
            if (alerts.length) {
