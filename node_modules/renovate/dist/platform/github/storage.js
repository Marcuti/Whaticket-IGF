    ...
    constructor() {
        ...
        async function getBranchList() {
            if (!branchList) {
                ...
                branchList = (await get(`repos/${config.repository}/branches?per_page=100`, {
                    paginate: true,
        ...
        async function getFileList(branchName) {
            ...
            try {
                const res = await get(`repos/${config.repository}/git/trees/${branch}?recursive=true`);
                if (res.body.truncated) {
                    logger.warn({ repository: config.repository }, 'repository tree is truncated');
                }
                ...
                logger.info({ branchName, err, repository: config.repository }, 'Error retrieving git tree - no files detected');
                throw err;
            }
        }
        async function getAllRenovateBranches(branchPrefix) {
            ...
            if (branchPrefix.endsWith('/')) {
                ...
                if (allBranches.includes(branchPrefixPrefix)) {
                    logger.warn(`Pruning branch "${branchPrefixPrefix}" so that it does not block PRs`);
                    await deleteBranch(branchPrefixPrefix);
                }
            }
            return allBranches.filter(branchName => branchName.startsWith(branchPrefix));
        }
        ...
        async function deleteBranch(branchName) {
            ...
            const options = config.forkToken
                ...
                : undefined;
            try {
                await get.delete(`repos/${config.repository}/git/refs/heads/${branchName}`, options);
            }
            catch (err) /* istanbul ignore next */ {
                if (err.message.startsWith('Reference does not exist')) {
                    logger.info({ branch: branchName }, 'Branch to delete does not exist');
                }
        ...
        async function mergeBranch(branchName) {
            ...
            try {
                ...
                if (err.message.startsWith('Required status check') ||
        ...
        async function getBranchLastCommitTime(branchName) {
            try {
                const res = await get(`repos/${config.repository}/commits?sha=${branchName}`);
        ...
        async function getFile(filePath, branchName) {
            ...
            let res;
            try {
                res = await get(`repos/${config.repository}/contents/${encodeURI(filePath)}?ref=${branchName || config.baseBranch}`);
            }
            catch (error) {
                ...
                if (error.statusCode === 403 &&
                    ...
                    const treeUrl = `repos/${config.repository}/git/trees/${config.baseBranch}`;
        ...
        async function updateBranch(branchName, commit) {
            ...
            try {
                await get.patch(`repos/${config.repository}/git/refs/heads/${branchName}`, options);
            }
            catch (err) /* istanbul ignore next */ {
                if (err.statusCode === 422) {
