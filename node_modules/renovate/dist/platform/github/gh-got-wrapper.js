...
async function get(path, options, retries = 5) {
    ...
    try {
        ...
        if (opts.paginate) {
            ...
            if (linkHeader && linkHeader.next && linkHeader.last) {
                ...
                const queue = pageNumbers.map(page => () => {
                    ...
                    return get(URL.format(nextUrl), { ...opts, paginate: false }, retries);
                });
                const pages = await pAll(queue, { concurrency: 5 });
                res.body = res.body.concat(...pages.filter(Boolean).map(page => page.body));
            }
        }
        if (method === 'get' &&
        ...
        if (err.statusCode >= 500 && err.statusCode < 600) {
            if (retries > 0) {
                logger.info({ statusCode: err.statusCode, message: err.message }, `Retrying request`);
...
for (const x of helpers) {
    get[x] = (url, opts) => get(url, Object.assign({}, opts, { method: x.toUpperCase() }));
}
