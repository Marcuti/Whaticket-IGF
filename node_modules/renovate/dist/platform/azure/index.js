...
async function initRepo({ repository, localDir, azureWorkItemId }) {
    ...
    const url = defaults.endpoint.replace('https://', `https://token:${opts.token}@`) +
...
async function getBranchCommit(fullBranchName) {
    const azureApiGit = await azureApi.gitApi();
    const commit = await azureApiGit.getBranch(config.repoId, azureHelper.getBranchNameWithoutRefsheadsPrefix(fullBranchName));
...
async function findPr(branchName, prTitle, state = 'all') {
    ...
    try {
        ...
        // update format
        prsFiltered = prsFiltered.map(item => azureHelper.getRenovatePRFormat(item));
        switch (state) {
            case 'all':
...
async function createPr(branchName, title, body, labels, useDefaultBranch, platformOptions = {}) {
    ...
    const targetRefName = azureHelper.getNewBranchName(useDefaultBranch ? config.defaultBranch : config.baseBranch);
...
function setBranchStatus(branchName, context, description, state, targetUrl) {
    logger.debug(`setBranchStatus(${branchName}, ${context}, ${description}, ${state}, ${targetUrl}) - Not supported by Azure DevOps (yet!)`);
}
...
async function addAssignees(issueNo, assignees) {
    ...
    await ensureComment(issueNo, 'Add Assignees', assignees.map(a => `@<${a}>`).join(', '));
}
/**
 *
 * @param {number} prNo
...
function getPrFiles(prNo) {
    logger.info(`getPrFiles(prNo)(${prNo}) - Not supported by Azure DevOps (yet!)`);
    return [];
}
