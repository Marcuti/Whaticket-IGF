...
function getBranchNameWithoutRefsheadsPrefix(branchPath) {
    ...
    if (!branchPath.startsWith('refs/heads/')) {
        logger.trace(`The refs/heads/ name should have started with 'refs/heads/' but it didn't. (${branchPath})`);
        return branchPath;
    }
...
function getBranchNameWithoutRefsPrefix(branchPath) {
    ...
    if (!branchPath.startsWith('refs/')) {
        logger.trace(`The ref name should have started with 'refs/' but it didn't. (${branchPath})`);
        return branchPath;
    }
...
async function getRefs(repoId, branchName) {
    ...
    const refs = await azureApiGit.getRefs(repoId, null, getBranchNameWithoutRefsPrefix(branchName));
    return refs;
}
/**
 *
...
async function getChanges(files, repoId, repository, branchName) {
    ...
    for (const file of files) {
        // Add or update
        let changeType = 1;
        const fileAlreadyThere = await getFile(repoId, repository, file.name, branchName);
        if (fileAlreadyThere) {
            changeType = 2;
        }
        changes.push({
...
async function getFile(repoId, repository, filePath, branchName) {
    ...
    const azureApiGit = await azureApi.gitApi();
    const item = await azureApiGit.getItemText(repoId, filePath, null, null, 0, // because we look for 1 file
    false, false, true, {
        versionType: 0,
        versionOptions: 0,
