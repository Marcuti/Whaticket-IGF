...
async function setBranchStatus(branchName, context, description, state, targetUrl) {
    ...
    const options = {
        state: state.replace('failure', 'failed'),
        description,
        context,
    };
    if (targetUrl) {
    ...
    try {
        ...
        if (err.message &&
            err.message.startsWith('Cannot transition status via :enqueue from :pending')) {
...
async function findIssue(title) {
    ...
    try {
        ...
        const issueBody = (await get(`projects/${config.repository}/issues/${issue.iid}`)).body.description;
        return {
            number: issue.iid,
            body: issueBody,
        };
...
async function ensureIssue(title, body) {
    ...
    try {
        ...
        const issue = issueList.find(i => i.title === title);
        if (issue) {
            const existingDescription = (await get(`projects/${config.repository}/issues/${issue.iid}`)).body.description;
...
async function addComment(issueNo, body) {
    ...
    await get.post(`projects/${config.repository}/merge_requests/${issueNo}/notes`, {
        body: { body },
    });
}
async function editComment(issueNo, commentId, body) {
    // PUT projects/:owner/:repo/merge_requests/:number/notes/:id
    await get.put(`projects/${config.repository}/merge_requests/${issueNo}/notes/${commentId}`, {
        body: { body },
    });
}
async function deleteComment(issueNo, commentId) {
    // DELETE projects/:owner/:repo/merge_requests/:number/notes/:id
    await get.delete(`projects/${config.repository}/merge_requests/${issueNo}/notes/${commentId}`);
}
...
// Pull Request
async function createPr(branchName, title, description, labels, useDefaultBranch) {
...
async function getPr(iid) {
    ...
    const branchUrl = `projects/${config.repository}/repository/branches/${urlEscape(pr.source_branch)}`;
    try {
        const branch = (await get(branchUrl)).body;
        const branchCommitEmail = branch && branch.commit ? branch.commit.author_email : null;
        // istanbul ignore if
        if (branchCommitEmail === config.email) {
            pr.canRebase = true;
        }
        else {
            logger.debug({ branchCommitEmail, configEmail: config.email, iid: pr.iid }, 'Last committer to branch does not match bot email, so PR cannot be rebased.');
            pr.canRebase = false;
        }
    }
    catch (err) {
...
async function getPrFiles(mrNo) {
    ...
    const files = (await get(`projects/${config.repository}/merge_requests/${mrNo}/changes`)).body.changes;
