        ...
        for (const [key, val] of Object.entries(config)) {
            if (removedOptions.includes(key)) {
                ...
                if (is.array(val)) {
                    ...
                    const migratedPathRules = migratedConfig.pathRules.map(p => migrateConfig(p, key).migratedConfig);
                    migratedConfig.packageRules = migratedPathRules.concat(migratedConfig.packageRules);
                }
                ...
                isMigrated = true;
                migratedConfig.packageRules = migratedConfig.packageRules || [];
                migratedConfig.packageRules = migratedConfig.packageRules.concat(migratedConfig.packages.map(p => migrateConfig(p, key).migratedConfig));
                ...
                for (let i = 0; i < schedules.length; i += 1) {
                    ...
                    if (schedules[i].match(/every (mon|tues|wednes|thurs|fri|satur|sun)day$/)) {
                        isMigrated = true;
