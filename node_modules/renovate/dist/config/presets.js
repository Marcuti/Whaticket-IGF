...
async function resolveConfigPresets(inputConfig, ignorePresets, existingPresets = []) {
    if (!ignorePresets) {
        ignorePresets = inputConfig.ignorePresets || []; // eslint-disable-line
    }
    logger.trace({ config: inputConfig, existingPresets }, 'resolveConfigPresets');
    let config = {};
    ...
    if (inputConfig.extends && inputConfig.extends.length) {
        for (const preset of inputConfig.extends) {
            ...
            if (existingPresets.includes(preset)) {
                ...
                const presetConfig = await resolveConfigPresets(fetchedPreset, ignorePresets, existingPresets.concat([preset]));
                // istanbul ignore if
                if (inputConfig &&
        ...
        if (is.array(val)) {
            ...
            for (const element of val) {
                if (is.object(element)) {
                    config[key].push(await resolveConfigPresets(element, ignorePresets, existingPresets));
                }
                else {
            ...
            config[key] = await resolveConfigPresets(val, ignorePresets, existingPresets);
        }
    }
...
async function getPreset(preset) {
    ...
    let presetConfig = await datasources[datasource].getPreset(packageName, presetName);
