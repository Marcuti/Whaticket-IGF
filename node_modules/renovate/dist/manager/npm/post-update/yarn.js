...
async function generateLockFile(cwd, env, config = {}, upgrades = []) {
    ...
    try {
        ...
        try {
            ...
            const installedPath = upath.join(await getInstalledPath('yarn', {
                local: true,
            }), 'bin/yarn.js');
            cmd = `node ${installedPath}`;
            const yarnIntegrity = config.upgrades &&
            ...
            if (!yarnIntegrity) {
                try {
                    ...
                    cmd = cmd.replace('node_modules/yarn/bin/yarn.js', 'bin/yarn-1.9.4.js');
                }
            }
        }
        catch (localerr) {
            ...
            try {
                ...
                try {
                    const installedPath = upath.join(await getInstalledPath('yarn'), 'bin/yarn.js');
                    cmd = `node ${installedPath}`;
                }
                catch (globalerr) {
        ...
        logger.info({ seconds, type: 'yarn.lock', stdout, stderr }, 'Generated lockfile');
    }
