...
async function writeExistingFiles(config, packageFiles) {
    ...
    for (const packageFile of npmFiles) {
        const basedir = upath.join(config.localDir, path.dirname(packageFile.packageFile));
        logger.trace(`Writing package.json to ${basedir}`);
        // Massage the file to eliminate yarn errors
        const massagedFile = JSON.parse(await platform.getFile(packageFile.packageFile));
        if (massagedFile.name) {
            massagedFile.name = massagedFile.name.replace(/[{}]/g, '');
        }
        delete massagedFile.engines;
        delete massagedFile.scripts;
        await fs.outputFile(upath.join(basedir, 'package.json'), JSON.stringify(massagedFile));
        // copyLocalLibs
        ...
        if (toCopy.length !== 0) {
            ...
            await Promise.all(toCopy.map(async (localPath) => {
                const resolvedLocalPath = upath.join(path.resolve(basedir, localPath));
                if (!resolvedLocalPath.startsWith(upath.join(config.localDir))) {
                    logger.info(`local lib '${toCopy}' will not be copied because it's out of the repo.`);
                }
                else {
                    // at the root of local Lib we should find a package.json so that yarn/npm will use it to update *lock file
                    const resolvedRepoPath = upath.join(resolvedLocalPath.substring(config.localDir.length + 1), 'package.json');
                    const fileContent = await platform.getFile(resolvedRepoPath);
                    if (fileContent !== null) {
                        await fs.outputFile(upath.join(resolvedLocalPath, 'package.json'), fileContent);
                    }
                    else {
                        logger.info(`listOfNeededLocalFiles - file '${resolvedRepoPath}' not found.`);
                    }
                }
            }));
        }
        ...
        if (packageFile.yarnrc) {
            ...
            await fs.outputFile(upath.join(basedir, '.yarnrc'), packageFile.yarnrc
        ...
        for (const [libName, libVersion] of Object.entries(dependencies)) {
            if (is.string(libVersion) && libVersion.startsWith('file:')) {
                if (libVersion.endsWith('.tgz')) {
                    logger.info(`Link to local lib "${libName}": "${libVersion}" is not supported. Please do it like: 'file:/path/to/folder'`);
                }
                else {
...
async function writeUpdatedPackageFiles(config) {
    ...
    for (const packageFile of config.updatedPackageFiles) {
        ...
        await fs.outputFile(upath.join(config.localDir, packageFile.name), JSON.stringify(massagedFile));
    }
}
// istanbul ignore next
async function getAdditionalFiles(config, packageFiles) {
    ...
    const env = global.trustLevel === 'high'
        ? process.env
        : {
    ...
    for (const lockFile of dirs.npmLockDirs) {
        ...
        const res = await npm.generateLockFile(upath.join(config.localDir, lockFileDir), env, fileName, config, upgrades);
        if (res.error) {
            // istanbul ignore if
            if (res.stderr && res.stderr.includes('No matching version found for')) {
                for (const upgrade of config.upgrades) {
                    if (res.stderr.includes(`No matching version found for ${upgrade.depName}`)) {
                        logger.info({ dependency: upgrade.depName, type: 'npm' }, 'lock file failed for the dependency being updated - skipping branch creation');
            ...
            const existingContent = await platform.getFile(lockFile, config.parentBranch);
    ...
    for (const lockFile of dirs.yarnLockDirs) {
        ...
        const res = await yarn.generateLockFile(upath.join(config.localDir, lockFileDir), env, config, upgrades);
        if (res.error) {
            // istanbul ignore if
            if (res.stderr && res.stderr.includes(`Couldn't find any versions for`)) {
                for (const upgrade of config.upgrades) {
                    ...
                    if (res.stderr.includes(`Couldn't find any versions for \\\"${upgrade.depName}\\\"`)) {
                        logger.info({ dependency: upgrade.depName, type: 'yarn' }, 'lock file failed for the dependency being updated - skipping branch creation');
                        throw new Error('registry-failure');
                    }
            ...
            const existingContent = await platform.getFile(lockFileName, config.parentBranch);
            if (res.lockFile !== existingContent) {
    ...
    for (const lockFile of dirs.pnpmShrinkwrapDirs) {
        ...
        const res = await pnpm.generateLockFile(upath.join(config.localDir, lockFileDir), env, config.binarySource);
        if (res.error) {
            // istanbul ignore if
            if (res.stdout && res.stdout.includes(`No compatible version found:`)) {
                for (const upgrade of config.upgrades) {
                    if (res.stdout.includes(`No compatible version found: ${upgrade.depName}`)) {
                        logger.info({ dependency: upgrade.depName, type: 'pnpm' }, 'lock file failed for the dependency being updated - skipping branch creation');
                        throw new Error('registry-failure');
                    }
                }
            }
    ...
    for (const lernaDir of dirs.lernaDirs) {
        ...
        const lernaPackageFile = packageFiles.npm.find(p => path.dirname(p.packageFile) === lernaDir);
        if (!lernaPackageFile) {
        ...
        const res = await lerna.generateLockFiles(lernaPackageFile.lernaClient, upath.join(config.localDir, lernaDir), env, skipInstalls, config.binarySource);
        ...
        if (res.error) {
            ...
            for (const packageFile of packageFiles.npm) {
                ...
                const existingContent = await platform.getFile(filename, config.parentBranch);
                if (existingContent) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            newContent = await fs.readFile(lockFilePath.replace('npm-shrinkwrap.json', 'package-lock.json'));
                        }
                        ...
                        if (config.updateType === 'lockFileMaintenance') {
                            logger.info({ packageFile, lockFilePath }, 'No lock file found after lerna lockFileMaintenance');
                        }
                        else {
                            logger.warn({ packageFile, lockFilePath }, 'No lock file found after lerna bootstrap');
                        }
                    }
                }
                else {
