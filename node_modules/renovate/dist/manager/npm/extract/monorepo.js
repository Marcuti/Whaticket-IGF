...
function matchesAnyPattern(val, patterns) {
    const res = patterns.some(pattern => pattern === val + '/' || minimatch(val, pattern, { dot: true }));
...
function detectMonorepos(packageFiles) {
    ...
    for (const p of packageFiles) {
        ...
        if (packages && packages.length) {
            logger.debug({ packageFile, yarnWorkspacesPackages, lernaPackages }, 'Found monorepo packages with base path ' + JSON.stringify(basePath));
            const internalPackagePatterns = (is.array(packages)
                ? packages
                : [packages]).map(pattern => upath.join(basePath, pattern));
            const internalPackageFiles = packageFiles.filter(sp => matchesAnyPattern(path.dirname(sp.packageFile), internalPackagePatterns));
            ...
            for (const subPackage of internalPackageFiles) {
                subPackage.internalPackages = internalPackages.filter(name => name !== subPackage.packageJsonName);
