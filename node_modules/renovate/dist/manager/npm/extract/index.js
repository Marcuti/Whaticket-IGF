...
async function extractPackageFile(content, fileName, config) {
    ...
    logger.debug(`npm file ${fileName} has name ${JSON.stringify(packageJsonName)}`);
    ...
    const yarnrc = (await platform.getFile(upath.join(path.dirname(fileName), '.yarnrc'))) ||
        undefined;
    let lernaDir;
    let lernaPackages;
    let lernaClient;
    let hasFileRefs = false;
    const lernaJson = JSON.parse(await platform.getFile(upath.join(path.dirname(fileName), 'lerna.json')));
    if (lernaJson) {
    ...
    function extractDependency(depType, depName, input) {
        ...
        if (!githubOwner.match(githubValidRegex) ||
        ...
        else if (depRefPart.match(/^[0-9a-f]{7}$/) ||
