...
async function getArtifacts(packageFileName, updatedDeps, newPackageFileContent, config) {
    ...
    try {
                ...
                for (const regUrl of config.registryUrls) {
                    if (regUrl.url) {
                        ...
                        if (hostRule) {
                            ...
                            logger.debug('No packagist auth found for host ' + host);
                        }
                    }
                }
            }
            else if (config.registryUrls) {
                logger.warn({ registryUrls: config.registryUrls }, 'Non-array composer registryUrls');
            }
        }
        ...
        const args = ('update ' + updatedDeps.join(' ')).trim() +
        ...
        logger.info({ seconds, type: 'composer.lock', stdout, stderr }, 'Generated lockfile');
        // istanbul ignore if
        if (config.gitFs) {
        ...
        if (err.message &&
            err.message.includes('Your requirements could not be resolved to an installable set of packages.')) {
