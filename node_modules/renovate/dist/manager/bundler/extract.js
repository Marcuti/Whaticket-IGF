...
async function extractPackageFile(content, fileName) {
    ...
    const delimeter = content.split('"').length > content.split("'").length ? '"' : "'";
    if (fileName) {
        logger.debug({ fileName }, 'Using delimeter ' + delimeter + ' to parse ' + fileName);
    }
    const lines = content.split('\n');
    for (let lineNumber = 0; lineNumber < lines.length; lineNumber += 1) {
        const line = lines[lineNumber];
        const sourceMatch = line.match(new RegExp(`^source ${delimeter}([^${delimeter}]+)${delimeter}\\s*$`));
        if (sourceMatch) {
            res.registryUrls.push(sourceMatch[1]);
        }
        const rubyMatch = line.match(new RegExp(`^ruby ${delimeter}([^${delimeter}]+)${delimeter}`));
        if (rubyMatch) {
        ...
        const sourceBlockMatch = line.match(new RegExp(`^source\\s+${delimeter}(.*?)${delimeter}\\s+do`));
        if (sourceBlockMatch) {
