...
function updateDependency(currentFileContent, upgrade) {
    try {
        const delimiter = currentFileContent.split('"').length >
        ...
        const newValue = upgrade.newValue
            ...
            .join('');
        const newLine = lineToChange.replace(new RegExp(`(gem ${delimiter}[^${delimiter}]+${delimiter})(,\\s+${delimiter}[^${delimiter}]+${delimiter}){0,2}`), `$1${newValue}`);
