...
const { isValid, isVersion, matches, sortVersions, } = require('../../versioning/ruby');
module.exports = {
    getArtifacts,
};
// istanbul ignore next
async function getArtifacts(packageFileName, updatedDeps, newPackageFileContent, config) {
    ...
    try {
        ...
        if (config.binarySource === 'docker') {
            ...
            if (config && config.compatibility && config.compatibility.ruby) {
                ...
                const rubyVersionFile = upath.join(upath.dirname(packageFileName), '.ruby-version');
            ...
            if (rubyConstraint && isValid(rubyConstraint)) {
                ...
                if (tag === 'latest') {
                    logger.warn({ rubyConstraint }, 'Failed to find a tag satisfying ruby constraint, using latest ruby image instead');
                }
            }
            const bundlerConstraint = config && config.compatibility && config.compatibility.bundler
        ...
        logger.info({ seconds, type: 'Gemfile.lock', stdout, stderr }, 'Generated lockfile');
        // istanbul ignore if
        if (config.gitFs) {
        ...
        if (err.stdout &&
            ...
            !config.gitFs) {
            logger.warn({ err }, 'It is necessary to run Renovate in gitFs mode - contact your bot administrator');
        ...
        if ((err.stdout &&
            ...
            logger.warn({ err }, 'Gemfile.lock update failed due to missing credentials');
        ...
        if (err.stderr && err.stderr.includes('incompatible marshal file format')) {
            ...
            logger.debug({ err, gemfile: newPackageFileContent, gemrcFile }, 'Gemfile marshalling error');
