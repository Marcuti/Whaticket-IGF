    ...
    try {
        ...
        if (upgrade.depType === 'git_repository' ||
            ...
            try {
                ...
                logger.debug('Failed to download release download - trying archive instead');
            ...
            const hash = crypto
                ...
                .update(file)
                .digest('hex');
            const valuePattern = new RegExp(upgrade.currentValue.replace(/\./g, '\\.'), 'g');
            ...
            newDef = newDef.replace(new RegExp(`(strip_prefix\\s*=\\s*)"[^"]*"`), `$1"${shortRepo}-${upgrade.newDigest}"`);
