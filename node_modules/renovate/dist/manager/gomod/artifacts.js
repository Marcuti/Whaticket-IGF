...
async function getArtifacts(goModFileName, updatedDeps, newGoModContent, config) {
    ...
    try {
        ...
        if (config.binarySource === 'docker') {
            ...
            if (credentials && credentials.token) {
                ...
                cmd += `bash -c "git config --global url.\\"https://${global.appMode
                    ? `x-access-token:${credentials.token}`
                    : credentials.token}@github.com/\\".insteadOf \\"https://github.com/\\" && go`;
            }
            else {
                cmd += 'go';
            }
        ...
        logger.info({ seconds, type: 'go.sum', stdout, stderr }, 'Generated lockfile');
        // istanbul ignore if
        if (config.postUpdateOptions &&
            ...
            if (config.gitFs) {
                ...
                logger.info({ seconds, stdout, stderr }, 'Tidied Go Modules after update');
            }
            else {
                logger.warn('Renovate administrator should enable gitFs in order to support tidying go modules');
            }
        }
        const res = [];
        // istanbul ignore if
        ...
        if (await platform.getFile(vendorModulesFileName)) {
            if (config.gitFs) {
                ...
                if (config.postUpdateOptions &&
                    ...
                    duration = process.hrtime(startTime);
                    seconds = Math.round(duration[0] + duration[1] / 1e9);
                    logger.info({ seconds, stdout, stderr }, 'Tidied Go Modules after vendoring');
                }
                const status = await platform.getRepoStatus();
                ...
                logger.warn('Vendor modules found - Renovate administrator should enable gitFs in order to update them');
            }
        }
        const finalGoModContent = (await fs.readFile(localGoModFileName, 'utf8')).replace(/\/\/ renovate-replace /g, '');
