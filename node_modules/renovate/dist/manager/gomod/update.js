...
function updateDependency(currentFileContent, upgrade) {
    try {
        ...
        if (!lineToChange.includes(depNameNoVersion) &&
            ...
            logger.debug({ lineToChange, depName }, "go.mod current line doesn't contain dependency");
            return null;
        }
        let requireLine;
        ...
        if (upgrade.updateType === 'digest') {
            ...
            logger.debug({ depName, lineToChange, newDigestRightSized }, 'gomod: need to update digest');
        ...
        if (upgrade.updateType === 'major') {
            ...
            if (depName.startsWith('gopkg.in/')) {
                ...
                if (upgrade.currentValue.match(/^v(0|1)\./)) {
                    ...
                    newLine = newLine.replace(new RegExp(`/${oldV}(\\s+)`), `/v${upgrade.newMajor}$1`);
                }
            }
        }
