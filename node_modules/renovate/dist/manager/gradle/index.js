...
const GRADLE_DEPENDENCY_REPORT_OPTIONS = '--init-script renovate-plugin.gradle renovate';
...
async function extractAllPackageFiles(config, packageFiles) {
    ...
    gradle.init();
    const dependencies = await updatesReport.extractDependenciesFromUpdatesReport(config.localDir);
...
function updateDependency(fileContent, upgrade) {
    ...
    return gradle.updateGradleVersion(fileContent, buildGradleDependency(upgrade), upgrade.newValue);
}
...
async function executeGradle(config) {
    let stdout;
    let stderr;
    const gradleTimeout = config.gradle && config.gradle.timeout
...
async function getGradleCommandLine(config) {
    ...
    if (gradlewExists && !config.gitFs) {
        logger.warn('Found gradle wrapper (gradlew) in the project, but --git-fs is not enabled, not using it.');
    }
