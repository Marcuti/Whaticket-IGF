...
function updateVersionMapVariableFormat(dependency, buildGradleContent, newVersion) {
    const regex = moduleMapVariableVersionFormatMatch(dependency);
    const match = buildGradleContent.match(regex);
    if (match) {
        return buildGradleContent.replace(variableDefinitionFormatMatch(match[1]), `$1${newVersion}$3`);
    }
    return null;
}
function updateVersionStringVariableFormat(dependency, buildGradleContent, newVersion) {
...
function updateVersionExpressionVariableFormat(dependency, buildGradleContent, newVersion) {
...
function updateGlobalVariables(dependency, buildGradleContent, newVersion) {
    ...
    if (variable) {
        ...
        const match = buildGradleContent.match(regex);
        if (match) {
            return buildGradleContent.replace(variableDefinitionFormatMatch(variable), `$1${newVersion}$3`);
        }
    }
    return null;
}
...
function moduleMapVersionFormatMatch(dependency) {
    // prettier-ignore
    return new RegExp(`(group\\s*:\\s*["']${dependency.group}["']\\s*,\\s*` +
