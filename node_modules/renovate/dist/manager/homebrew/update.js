...
async function updateDependency(content, upgrade) {
    ...
    if (!oldParsedUrlPath) {
        logger.debug(`Failed to update - upgrade.url is invalid ${upgrade.depName}`);
        return content;
    }
    try {
        newUrl = `https://github.com/${upgrade.ownerName}/${upgrade.repoName}/releases/download/${upgrade.newValue}/${upgrade.repoName}-${coerce(upgrade.newValue)}.tar.gz`;
        ...
        logger.debug(`Failed to download release download for ${upgrade.depName} - trying archive instead`);
        try {
            newUrl = `https://github.com/${upgrade.ownerName}/${upgrade.repoName}/archive/${upgrade.newValue}.tar.gz`;
            file = (await got(newUrl, { encoding: null })).body;
        }
        catch (errInner) {
            logger.debug(`Failed to download archive download for ${upgrade.depName} - update failed`);
            return content;
        }
    }
