...
async function extractSetupFile(content, packageFile, config) {
    ...
    if (config.binarySource === 'docker') {
        ...
        cmd = 'docker';
        args.unshift('run', '-i', '--rm', 
        // volume
        '-v', `${cwd}:${cwd}`, '-v', `${__dirname}:${__dirname}`, 
        // cwd
        '-w', cwd, 
        // image
        'renovate/pip', 'python');
    ...
    try {
        ...
        if (err.message &&
            ...
            !config.gitFs) {
            logger.warn('File not found error when extracting setup.py. Ask your Renovate administrator to enable gitFs and try again');
        }
        throw err;
    }
    // istanbul ignore if
