...
function updateDependency(fileContent, upgrade) {
    ...
    if (!parsedContents.tool.poetry[depType]) {
        logger.info({ config: upgrade }, `Error: Section tool.poetry.${depType} doesn't exist in pyproject.toml file, update failed`);
        return null;
    }
    let oldVersion;
    if (nestedVersion) {
        ...
        if (!oldDep) {
            logger.info({ config: upgrade }, `Could not get version of dependency ${depType}, update failed (most likely name is invalid)`);
            return null;
        }
