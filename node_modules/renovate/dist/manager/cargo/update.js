...
function updateDependency(fileContent, upgrade) {
    ...
    if (!section) {
        if (target) {
            logger.info({ config: upgrade }, `Error: Section [target.${target}.${depType}] doesn't exist in Cargo.toml file, update failed`);
        }
        else {
            logger.info({ config: upgrade }, `Error: Section [${depType}] doesn't exist in Cargo.toml file, update failed`);
        }
        return fileContent;
    }
    let oldVersion;
    ...
    if (!oldDep) {
        logger.info({ config: upgrade }, `Could not get version of dependency ${depName}, update failed (most likely name is invalid)`);
        return fileContent;
    }
